...
Release: https://github.com/danielrna/enterprise-modernization-platform/releases/tag/v0.1.4

Sample smoke-test report: https://github.com/danielrna/enterprise-modernization-platform/releases/download/v0.1.4/emp-smoke-report.zip
...
```bash
docker run --rm -v "$PWD:/workspace" danielrna/enterprise-modernization-platform:v0.1.4 analyze . --pack spring-boot-3-readiness --out reports/docker-readiness
```
  ...
  readiness:
    ...
    steps:
      ...
      - name: Run EMP readiness
        uses: danielrna/enterprise-modernization-platform@v0.1.4
        with:
          path: .
...
The public validation set now proves the reference flow on 8 real checkouts, including real Spring Boot applications and Hibernate ORM evidence outside Spring Guides. The validated set includes Spring Boot `2.6.2`, `2.6.3`, and `2.7.6` projects plus a Hibernate ORM demo with compile `passed`, tests `passed`, and validation confidence `95%`.
...
- Validation status in benchmark reports and the Migration Hub, including 8 checkout benchmarks with passing compile/test evidence.
